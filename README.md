...
├── scripts/
│   └── windows-preflight.ps1       # Windows/Git Bash readiness check for Codex validation
├── docs/
...
## What's New in v2.21.31

**Theme: Windows Git Bash preflight for Codex validation (#323)**

- **Windows preflight script** — added `scripts/windows-preflight.ps1` to check `codex`, `git`, `node`, Git Bash, installed plugin listing, and the common PowerShell `bash` collision where `C:\Windows\System32\bash.exe` points at WSL without an installed distribution.
- **Single validator source of truth** — docs now state that Windows uses Git Bash as the compatibility layer for existing Bash validators, rather than maintaining parallel PowerShell validator implementations.
- **Copy-paste validator commands** — the preflight prints canonical Git Bash invocations for `validate-structure`, `validate-content`, `test-skill-graph`, and `test-verbosity-hook`.
...
## What's New in v2.21.29

**Theme: Trap-naming taxonomy lens for `/scrutinize` + `/cross-verify` (#319)**

- **New 14th integrity commandment** — "Never let a seductive-but-broken option pass unnamed" with a 4-mode failure taxonomy (hidden cost / false economy / scaling failure / premature abstraction), added to `guides/integrity-principles.md` under a "Design Judgment" heading. Inspired by [UditAkhourii/adhd](https://github.com/UditAkhourii/adhd)'s evidence that _naming_ traps is the highest-leverage adversarial output ([#319](https://github.com/pitimon/8-habit-ai-dev/issues/319)).
- **`/scrutinize` Step 1** now tags each rejected alternative with its failure mode, not just names a better one.
- **`/cross-verify` Shadow Self-Check** now tests whether the recommendation _itself_ is a trap against the same taxonomy.
- **Zero new surface** — both skills already `Load` the integrity guide, so no new Load lines, no `llms.txt` change; markdown-only, no frontmatter/runtime changes (8-habit discipline, not enforcement).
...
## What's New in v2.21.26

**Theme: Codebase-Orientation Skill — Out-of-Scope Record**

- **#311 closed as T3 (reject)** — the deferred "Idea A" from the Understand-Anything research (a `/understand-onboard`-style codebase-orientation skill) is documented in `docs/out-of-scope/codebase-orientation-skill.md` rather than shipped.
- **Why** — redundant with the installed `claude-mem` suite (pathfinder / learn-codebase / smart-explore), low predicted-use frequency, and the only distinct angle (H5 discipline) fits a guide paragraph. Record includes reconsideration conditions.
- **H8 / ADR-018** — shrink-not-grow surface; the documented "why not" is higher-leverage than the skill.
...
## What's New in v2.21.24

**Theme: Cross-Platform Frontmatter Rule + Disk-Intermediate Orchestration Precedent**

Two engineering lessons mined from [`Egonex-AI/Understand-Anything`](https://github.com/Egonex-AI/Understand-Anything) (58k★) via `/research deep`, folded into existing guides — **no new skill** (ADR-018 "earn each line").

- **Frontmatter portability rule** — `guides/vendor-portability.md` now bans the Claude-only `model: inherit` keyword (non-Claude runtimes reject it as `ProviderModelNotFoundError`) while preserving deliberate, documented model pins. Reviewer agents stay `model: opus` per [#285](https://github.com/pitimon/8-habit-ai-dev/issues/285).
- **Disk-intermediate orchestration precedent** — `guides/orchestration-patterns.md` Pattern 2 cites passing sub-agent results through disk rather than the parent context, reinforcing the context-economy principle.
- **Boundary preserved** — guide content only; a candidate codebase-orientation `/research` mode was deferred as a documented gap after an `8-habit-reviewer` cross-verify.
...
**Theme: Fable Model Review Record**

v2.21.22 adds the first standalone review record under `docs/reviews/`.

- **Review record** — full 8-Habit review of v2.21.21 by the Fable model: score 13/17, verdict fix-then-ship, 21 evidence-cited findings with habit mapping and confidence levels. See `docs/reviews/2026-06-10-fable-model-review.md`.
- **New convention** — `docs/reviews/` is now the home for standalone review records (release self-checks remain in `SELF-CHECK.md`).
- **Boundary preserved** — documentation only; no skill, hook, or validator behavior changes.
...
**Theme: AI dev log snapshot and trailer fallback**

v2.21.19 makes `/ai-dev-log` stable and honest for audit reports that depend on `Co-Authored-By` trailers.

- **Trailer fallback** — the generator uses Git's trailer parser first, then scans commit bodies only for valid `Co-Authored-By:` lines when the parser returns empty.
- **Snapshot boundary** — reports record the `HEAD` snapshot used for statistics and support `--snapshot <sha>` for exact reproduction.
- **Empty-state reports** — `--out` now writes a 0-detected report with limitations instead of exiting before creating audit evidence.
- **Boundary retained** — generator behavior only; no new skill, runtime enforcement, policy gate, hook behavior, or diff/file-content inspection.
...
- **High-stakes confidence footer** — `/cross-verify` now requires the `V/I/U` confidence footer for high-stakes reviews and records open unknowns explicitly.
- **Shadow Self-Check** — `/cross-verify` now asks for the strongest counter-argument to its own verdict and who is harmed if the verdict is wrong before recommending action.
- **Research template alignment** — research briefs gain a mandatory Confidence & Open Unknowns block, and integrity principle #7 now uses the shared `V/I/U` vocabulary.
- **Boundary retained** — markdown guidance only; no runtime enforcement, hooks, orchestration engine, policy gate, or duplicated `/scrutinize` gate.
...
- **Research scan** — `/research` now asks what AI acceleration may amplify: review load, validation cost, handoff drift, internal contracts, and human attention.
- **Design label** — `/design` uses `software ecology impact` only when acceleration changes boundaries, API contracts, validation, release, or ownership.
- **Review signals** — `/review-ai` now checks for review burden, validator brittleness, generated-content noise, and contract drift.
- **Boundary retained** — markdown guidance only; no runtime enforcement, hooks, orchestration engine, policy gate, or hook parity claim.
...
- **Existing-system mode** — requirements for real systems should cite source evidence where available and keep unverifiable statements visible as assumptions or questions.
- **Idea-mode** — raw ideas preserve intent and label assumptions before turning them into constraints.
- **Mixed work guard** — confirmed system facts stay separate from proposed behavior so `/design` does not inherit speculative claims as architecture truth.
- **Validator pins** — content validation now checks the new mode names and evidence/assumption discipline.
...
- **Dead code removed** — `BRIEF` and `PROGRESS` assignments are gone from `hooks/session-start.sh`.
- **Behavior preserved** — workflow artifact markers still render through the existing inline `${PRD:+ ✓}` / `${ADR:+ ✓}` / `${TASKS:+ ✓}` expressions.
- **Regression coverage retained** — `tests/test-verbosity-hook.sh` continues to cover root fallback, Codex JSON output, version rendering, and workflow hints.
...
- **Version fallback** — `hooks/session-start.sh` now reads `.claude-plugin/plugin.json` first, then `.codex-plugin/plugin.json`.
- **No `vunknown`** — installed Codex cache smoke now renders `8-Habit AI Dev Active (v2.21.14)` instead of `vunknown`.
- **Regression coverage** — `tests/test-verbosity-hook.sh` now builds a fake Codex-only package root and executes the registered SessionStart command against it.
...
- **Root fallback** — `hooks/hooks.json` now resolves the package root from `CLAUDE_PLUGIN_ROOT`, then `CODEX_MANAGED_PACKAGE_ROOT`, then the current package directory.
- **Behavior preserved** — the existing markdown reminder and Codex JSON adapter are unchanged; this is command-path compatibility only.
- **Regression coverage** — `tests/test-verbosity-hook.sh` now executes the registered hook command with `CLAUDE_PLUGIN_ROOT` unset.
...
- **Design walkthrough** — the Step 2 Design wiki page now includes a concrete claim-discipline example with pass level, claim labels, evidence strength, `Verify first: Yes/No`, question priority, and Mermaid traceability guidance.
- **Validator polish** — `/diagnose` and `/research` Definition of Done lists are compacted to remove existing validator warnings without changing their behavior.
- **Skill complexity polish** — `/ai-dev-log` now keeps script internals and report-template detail in `reference.md`, reducing the main skill body while preserving customization guidance.
- **Boundary held** — no new skill, runtime enforcement, hook behavior change, or semantic change to `/diagnose`, `/research`, or `/ai-dev-log`.
...
- **Right-sized design passes** — `/design` now chooses `Scan`, `Focus`, or `Full` and promotes only when evidence, explicit user request, or the risk of staying smaller justifies more scope.
- **Claim traceability** — load-bearing architecture claims now carry labels, evidence strength, and `Verify first: Yes/No` so assumptions and proposals do not masquerade as confirmed system facts.
- **ADR support** — the ADR template adds optional Architecture Claims and Decisions Requiring Approval sections for handoff-ready architecture decisions.
...
- **Claude Code install fix** — replaces the root symlink with a real `plugin/` child directory so Claude Code no longer packages a self-referential root link.
- **Codex compatibility preserved** — `plugin/.codex-plugin/plugin.json` keeps the required child-source shape for Codex marketplace installs and ships a real `plugin/skills/` copy so installed caches contain the skills.
- **Validator coverage** — `tests/validate-structure.sh` now fails if `plugin` is a symlink, checks the child Codex manifest, and detects drift between the root skill corpus and the packaged child copy.
...
- **Project context contract** — [`guides/project-context-contract.md`](guides/project-context-contract.md) defines optional repo-local files for glossary, issue tracker, triage labels, domain-doc routing, and ADR awareness.
- **Skill awareness** — `/requirements`, `/design`, `/breakdown`, `/build-brief`, `/diagnose`, and `/scrutinize` now read those context files when present instead of guessing project vocabulary or issue readiness.
- **Delegation readiness** — `/breakdown` and the AGENT-BRIEF template now distinguish `ready-for-agent`, `ready-for-human`, and `needs-info` before backlog-bound work is filed.
- **Boundary** — markdown guidance only: no setup engine, no automatic issue mutation, no runtime enforcement, and no Claude hook port to Codex.
...
- **Fast re-entry** — adds `SPEC.md` as a project digest and `DOMAIN.md` as the invariant/safety-boundary summary.
- **ADR discoverability** — adds `docs/adr/README.md` and ADR-025 to record why these context files exist.
- **Codex setup note** — adds `.codex/README.md` for repo-local Codex setup and the Claude-hook boundary.
- **LLM index** — updates `llms.txt` so remote agents can discover the new context files.
...
- **Installed-cache shape** — Check 28 now treats a missing `plugin` child source as expected only under `~/.codex/plugins/cache/...`, where Codex has already copied the plugin root.
- **Source/marketplace strictness preserved** — source and marketplace snapshots still require a real `plugin/` child source because `.agents/plugins/marketplace.json` points at `./plugin`.
- **Docs clarified** — `docs/codex-integration.md` and `docs/compatibility-matrix.md` now explain when to validate source/marketplace publishability versus installed-cache user artifacts.
...
- **Limitations page** — wiki documentation now states what the plugin helps with, what it does not enforce, and which evidence belongs in release/PR proof.
- **Runtime comparison matrix** — `docs/compatibility-matrix.md` now compares shared markdown skills, Claude Code packaging, Codex packaging, hooks, memory, enforcement, and release evidence across host platforms.
- **Real behavior proof** — `CONTRIBUTING.md` and the release checklist now require concrete validator/catalog/install evidence when user-facing surfaces change.
- **Validator coverage** — `tests/validate-structure.sh` now treats the wiki Limitations page as part of the required wiki skeleton.
- **Skill-effectiveness maintenance** — harvested one previously untallied lesson signal for `/whole-person-check` and aligned the report with the current 24-skill inventory.
...
- **Codex marketplace** — `.agents/plugins/marketplace.json` lets users install from the Git repo with `codex plugin marketplace add pitimon/8-habit-ai-dev` then `codex plugin add 8-habit-ai-dev@pitimon-8-habit-ai-dev`. It points to the real `./plugin` child directory because Codex ignores root-path marketplace entries.
...
- **Version convention** — release version now lives in 6 files: Claude manifest, Claude marketplace, source Codex manifest, child Codex manifest, README, and SELF-CHECK.
