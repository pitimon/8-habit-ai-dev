...
17. **[Optional] FR-017**: Where the user supplies a second positional argument to `/save-spec`, the skill shall resolve that path as the **target directory** for the scaffold. All subsequent file operations (pre-flight existence check from FR-002, glob detection from FR-004, write from FR-010, summary from step 8) shall operate on `<target-dir>` instead of the current working directory. If `<target-dir>` does not exist or is not a directory, the skill shall emit the canonical **pre-flight error template** defined in `skills/save-spec/reference.md` ("Canonical pre-flight error template (FR-017, target-dir validation)" section — distinct from the Decision-4 Write-failure register, which uses "Tried to create" wording inappropriate for path-not-found cases). The skill shall stop before any Write attempt. (F2 — issue [#203](https://github.com/pitimon/8-habit-ai-dev/issues/203); register-correction R5-2 — issue [#205](https://github.com/pitimon/8-habit-ai-dev/issues/205).)
