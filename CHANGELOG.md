...
- Replace the root `plugin -> .` symlink with a real `plugin/` child directory so Claude Code installers on Linux do not fail with `ENOENT: no such file or directory, symlink`.
- Add `plugin/.codex-plugin/plugin.json` and a real `plugin/skills/` package copy for Codex's required child-source marketplace shape.

### Validation

- Extend `tests/validate-structure.sh` to validate the child Codex manifest, enforce version sync across the four manifest files plus README, fail if `plugin` is a symlink, and detect drift between the root corpus and the packaged child copy.

### Boundary

- Packaging compatibility only. No skill behavior change, no runtime enforcement, no policy authorization, and no Claude hook feature-parity claim for Codex.
...
- Fix `tests/validate-structure.sh` Check 28 when run from Codex's installed plugin cache. At the time, the validator kept `plugin -> .` mandatory in source/marketplace snapshots, but treated the missing symlink as expected under `~/.codex/plugins/cache/...`, where Codex had already copied the plugin root. Closes issue #278. Superseded by v2.21.10's real `plugin/` child-source directory.
...
- **`.agents/plugins/marketplace.json`** — Codex marketplace descriptor named `pitimon-8-habit-ai-dev`, with plugin source path `"./plugin"`. This originally used a `plugin -> .` symlink because Codex marketplace discovery ignores root-path entries (`"."` / `"./"`); v2.21.10 replaces that symlink with a real child directory to avoid Linux Claude Code install failures. Install flow: `codex plugin marketplace add pitimon/8-habit-ai-dev` then `codex plugin add 8-habit-ai-dev@pitimon-8-habit-ai-dev`.
