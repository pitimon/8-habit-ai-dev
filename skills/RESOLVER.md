...
| `"check spec consistency"`, `"PRD vs design vs tasks drift"`, `"incident consistency"`, `"config hotfix drift"`, `"spec-kit /analyze"` | [`skills/consistency-check/SKILL.md`](consistency-check/SKILL.md)             | Cross-artifact analyzer over persisted `docs/specs/<slug>/`, plus incident/config hotfix mode for symptom/evidence/root-cause/fix/verification drift (v2.20.1)               |
| `"operational state"`, `"watch or fix"`, `"active incident or resolved"`               | [`skills/operational-state/SKILL.md`](operational-state/SKILL.md)             | Classify operational findings before action — Watch, Fix Candidate, Active Incident, Resolved, Handoff, Known Accepted Issue, False Positive, Self-Resolved. H1 + H5 + H8    |
