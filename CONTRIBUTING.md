...
Version lives in **6 files** — all must be bumped together:
...
`tests/validate-structure.sh` enforces consistency across all six — CI fails if any drifts. See [issue #106](https://github.com/pitimon/8-habit-ai-dev/issues/106) for the drift incident that motivated the SELF-CHECK.md addition and [ADR-023](docs/adr/ADR-023-codex-native-packaging.md) for the Codex manifest addition.
